...
corrections as proposed in 1812.08163 - they are or are not added to
standard CKM matrix depending on the choice of CKMInput option in
SMInitializeModel routine (could be Yes, No or Force). Finally, PMNS
mixing is calculated using the SM relations, no EFT terms included as
of now.  *)
...
Module[{numf,numa,x},
numf = Rule @@@ Transpose[ {SM$FermionParameters[[All, 1]],
                            Value /. SM$FermionParameters[[All, 2]]} ];
numa = Rule @@@ Transpose[ {SM$AuxiliaryParameters[[All, 1]],
                            Value /. SM$AuxiliaryParameters[[All, 2]]} ];
...
(* all the user defined parameters should be real scalars, no indices or complex numbers *)
ScalarParStructureTest[ SM$InputParameters /. Equal -> List ];
...
Print["\nUsing ", Style["(G_F, MZ, MW, MH)", Bold], " input scheme for the EW sector... "];
...
Print["\nUsing ", Style["(alpha_em, MZ, MW, MH)", Bold], " input scheme for the EW sector... "];
...
vD6   = - MW^3/(4 Pi AEM MZ^2) (MW MBScalarList["phiD"] + 4 Sqrt[MZ^2 - MW^2] MBScalarList["phiWB"]);
...
numg = Rule @@@ Transpose[ {SM$GaugeParameters[[All, 1]],
                            Value /. SM$GaugeParameters[[All, 2]]} ];
numf = Rule @@@ Transpose[ {SM$FermionParameters[[All, 1]],
                            Value /. SM$FermionParameters[[All, 2]]} ];
numa = Rule @@@ Transpose[ {SM$AuxiliaryParameters[[All, 1]],
                            Value /. SM$AuxiliaryParameters[[All, 2]]} ];
...
If [ MemberQ[ {"yes", "force"}, SMEFT$CKMInput ] && CKMCorrectionsSize[],
...
UserInput$CKM = VCKMT - If[ MemberQ[ {"yes", "force"}, SMEFT$CKMInput ], DV, 0 ] /. SMEFT$WCValues;
...
to PMNS calculated as of now. In addition, PMNS rarely affects
phenomenology, so in UFO files by default it is set to be diagonal,
unless PMNSInput->"full" option is chosen in SMEFTInitializeModel
routine *)
