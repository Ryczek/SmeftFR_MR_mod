...
smvalues, mixnum, mixpar, x, y},

num = (#[[1]] -> Value /. #[[2]]) & /@  SM$InputParameters;
...
tmp = "UserInput$" <> # & /@ stdl;
(* line below non trivial if variables have numerical value pre-assigned *)
tmp = StringDrop[#,StringLength["UserInput$"]] & /@ Complement[ tmp, Intersection[ToString /@ (ToExpression /@ tmp), tmp] ];
sminput =(ToExpression[#] -> ToExpression["UserInput$" <> #]) & /@ tmp;

smnum = ToExpression["SMnum$" <> ToString[#]] -> ToExpression["UserInput$" <> ToString[#]] & /@ stdl;
smnum = (smnum //. Join[num, SMEFT$WCValues, sminput]) /. Complex[x_, 0.] -> x;
If[ ! NumberQ[Values[#]],
...
mixnum = Association[{ SMnum$Kq -> Flatten[ Table[ Kq[i,j] -> UserInput$CKM[[i,j]], {i,1,3},  {j,1,3} ] ],
...
mixpar = <|Kq -> smpar[Kq], Ul -> smpar[Ul]|>;

(* create separate variable with CKM and PMNS and drop them from rest of SM parameters list *)
tmp = Keys[smpar];
SMEFT$SMMixing = {SMEFT$SMParameters[[ Position[tmp, Kq][[1,1]] ]]};
SMEFT$SMParameters = Drop[ SMEFT$SMParameters, Position[tmp, Kq][[1]] ];
smpar = Drop[ smpar, Position[tmp, Kq][[1]] ];
tmp = Keys[smpar];
SMEFT$SMMixing = Append[ SMEFT$SMMixing, SMEFT$SMParameters[[ Position[tmp, Ul][[1,1]] ]] ];
SMEFT$SMParameters = Drop[ SMEFT$SMParameters, Position[tmp, Ul][[1]] ];
smpar = Drop[ smpar, Position[tmp, Ul][[1]] ];

(* update scalar parameters *)
smnames = Intersection[ ToString /@ Keys[smpar], stdl ];
smvalues = smpar[ToExpression[#]] /. (Value -> x_) ->
       (Value -> smnum[ToExpression["SMnum$" <> #]]) & /@ smnames;
smvalues = Association[ Rule @@@ Transpose[ {ToExpression /@ smnames, smvalues } ] ];
tmp = (ToExpression[#] == x_) -> (ToExpression[#] == smvalues[ToExpression[#]]) & /@ smnames;
SMEFT$SMParameters = SMEFT$SMParameters /. tmp;

(* update SM particle masses - separate because names need to be uppercase-d *)
...
(* update fermion mixing matrices *)
smnames = {"Kq","Ul"};
smvalues = mixpar[ToExpression[#]] /. (Value -> x_) -> (Value -> mixnum[ToExpression["SMnum$" <> #]]) & /@ smnames;
smvalues = Association[Rule @@@ Transpose[ {ToExpression /@ smnames, smvalues} ] ];
tmp = (ToExpression[#] == x_) -> (ToExpression[#] == smvalues[ToExpression[#]]) & /@ smnames;
SMEFT$SMMixing = SMEFT$SMMixing /. tmp;
...
num = Rule @@@ Transpose[ {SM$InputParameters[[All, 1]],
                           Value /. SM$InputParameters[[All, 2]] } ];
...
tmp = "UserInput$" <> # & /@ stdl;
tmp = StringDrop[#,StringLength["UserInput$"]] & /@ Complement[ tmp, Intersection[ToString /@ (ToExpression /@ tmp), tmp] ];
sminput = Rule @@@ Transpose[ {ToExpression /@ tmp,
                               ToExpression["UserInput$" <> #] & /@ tmp} ];
...
If [ ! SMEFT$InputListStatus && name != "NONE",
  ...
  alist = Rule @@@ Transpose[ {ToExpression /@ aznorm, alist} ];

  nlist = Join[clist, nlist, alist];

  clist = Keys[nlist];
  alist = Values[nlist] //. nlist;
  nlist = Rule @@@ Transpose[ {clist, alist} ];
...
tmp = StringDrop[#,StringLength["UserInput$"]] & /@ Complement[ tmp, Intersection[ToString /@ (ToExpression /@ tmp), tmp] ];
sinput = Rule @@@ Transpose[ {ToExpression /@ tmp, ToExpression["UserInput$" <> #] & /@ tmp} ];
...
svalues = Association[ Rule @@@ Transpose[ {ToExpression /@ snames, svalues} ] ];
