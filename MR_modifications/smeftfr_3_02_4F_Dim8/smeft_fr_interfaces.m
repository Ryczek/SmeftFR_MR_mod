...
SMEFT$Path = FileNameJoin[{$FeynRulesPath, "Models", "smeftfr_3_02_4F_Dim8"}];
...
WriteFeynArtsOutput[ SMEFT$MBLagrangian,
		     Output -> FileNameJoin[{SMEFT$Path, "output", "FeynArts", "SMEFT_4F_Dim6_Dim8_13_02"}] ];
Print["\nVertices stored in FeynArts format in directory ", 
       Style[FileNameJoin[{SMEFT$Path, "output", "SMEFT_4F_Dim6_Dim8_13_02"}] ,Bold] ];
...
SMEFTToUFO[ SMEFT$MBLagrangian,
	    Output -> FileNameJoin[{SMEFT$Path, "output", "UFO_SMEFT_4F_Dim8_psi4D2"}],
	    CorrectIO -> True ];
